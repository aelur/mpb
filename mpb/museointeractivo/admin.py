...
def validarExcel(excel, err):
	...
	if not hay_datos:
		err.append('El excel no tiene definido la hoja de jugadores, o campania o posiciones.')
	
	sjug = wb.sheet_by_name('jugadores')
	if (sjug.cell(0,0).value.upper() != 'JUGADOR' or
		sjug.cell(0,1).value.upper() != 'PARTIDOS' or
		sjug.cell(0,2).value.upper() != 'GOLES'):
		err.append('La planilla de Jugadores no tiene las columnas correctas (jugador,partidos,goles).')
	...
	if (scamp.cell(0,0).value.upper() != 'DIA' or
		...
		scamp.cell(0,4).value.upper() != 'CANCHA'):
		err.append('La planilla de Campania no tiene las columnas correctas (dia,jornada,rival,resultado,cancha).')
	...
	if (spos.cell(0,0).value.upper() != 'EQUIPOS' or
		...
		spos.cell(0,7).value.upper() != 'GC.'):
		err.append('La planilla de Posiciones no tiene las columnas correctas (equipos,ptos.,j.,g.,e.,p.,gf.,gc.).')
